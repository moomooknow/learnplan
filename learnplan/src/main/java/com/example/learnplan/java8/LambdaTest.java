...
public class LambdaTest {
    public static void main(String[] args) {
        ...
        * */
         ...
         * 类型推断：Lambda表达式中的参数类型都是由编译器推断得出的。 Lambda表达式中无需指定类型，程序依然可以编译，这是因为javac根据程序的上下文，在后台推断出了参数的类型。 Lambda 表达式的类型依赖于上下文环境，是由编译器推断出来的。
         * */
        Comparator<Integer> com2 = (Integer x, Integer y) -> {
            //参数类型可以省略 Integer
    ...
    public void listFor(){
        ...
        userList.stream().filter(p -> "ym".equals(p.getUserName())).limit(2).forEach(System.out::println);
        userList.stream().sorted(Comparator.comparingInt(User::getAge)).filter(p -> p.getAge()>17).forEach(System.out::println);
        //max 最大值如果有很多，只会返回一个
